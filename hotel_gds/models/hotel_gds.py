...
class hotel_reservation(models.Model):
    ...
    meal_id = fields.Selection([('no_meal', 'No Meal Included'), ('cont_break', 'Continental Breakfast'), ('buff_break', 'Buffet Breakfast'), (
        'half_board', 'Half-board'), ('full_board', 'Full-board')], 'Meal Type', readonly=True, states={'draft': [('readonly', False)]})
            ...
            for line in reservation.reservation_line:
                ...
                if config_browse and reservation.source != 'through_gds':
                    for record in config_browse:
                        ...
                        if (config_start_date <= reservation_start_date < config_end_date) or (config_start_date < reservation_end_date <= config_end_date) or ((reservation_start_date < config_start_date) and (reservation_end_date > config_end_date)):
                            for rec in record.line_ids:
                                ...
                                if line.room_number.id in room_list:
                                    raise Warning("Warning! Room  %s is reserved in this reservation period for GDS reservation !" % (
...
class hotel_room(models.Model):
    ...
    mon_bool = fields.Boolean('Monday')
    tue_bool = fields.Boolean('Tuesday')
    wed_bool = fields.Boolean('Wednesday')
    thr_bool = fields.Boolean('Thursday')
    fri_bool = fields.Boolean('Friday')
    sat_bool = fields.Boolean('Saturday')
    sun_bool = fields.Boolean('Sunday')
...
class hotel_reservation_through_gds_line(models.Model):
    ...
    room_number = fields.Many2many('product.product', 'hotel_gds_reservation_rel', 'product_id', 'line_id', 'Room',
                                   domain="[('isroom','=',True),('categ_id','=',categ_id)]", help="Will list out all the rooms that belong to selected shop.")
...
class hotel_reservation_line(models.Model):
    ...
    @api.depends('checkin', 'checkout', 'banquet_id.pricelist_id', 'banquet_id', 'line_id.partner_id', 'number_of_days', 'banquet_id.source')
    def onchange_room_id(self):
        ...
        if self.room_number:
            ...
            if pricelist is False:
                raise UserError("PriceList is not selected for the Resevation")
            ...
            #reservation_start_date = str(self.checkin)
            reservation_end_date = self.checkout
            #reservation_end_date = str(self.checkout)

            # print("reservation_start_date:::::::::::",reservation_start_date,reservation_end_date)
            config_id = self.env['hotel.reservation.through.gds.configuration'].search([])
            config_browse = config_id
                    ...
                    if (config_start_date <= reservation_start_date < config_end_date) or (config_start_date < reservation_end_date <= config_end_date) or ((reservation_start_date < config_start_date) and (reservation_end_date > config_end_date)):
                        ...
                        for line in record.line_ids:
                            ...
                            if self.room_number.id in room_list:
                                ...
                                raise Warning("Room  is reserved in this period for GDS reservation!")
            ...
            if housekeeping_room:
                for house1 in housekeeping_room:
                    ...
                    if (((start_reser < house_current_date) and (end_reser > house_end_date)) or (house_current_date <= start_reser < house_end_date) or (house_current_date < end_reser <= house_end_date)) and (house.state == 'dirty'):
                        raise Warning("Warning! Room  %s is not clean for reservation period !" % (room_line_id.name))
