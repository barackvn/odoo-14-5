...
class CrmLead2OpportunityPartner(models.TransientModel):
    ...
    def action_apply(self):
        ...
        if 'banquet_id' in self._context:
            banq_browse = self.env['banquet.quotation'].browse(self._context['banquet_id'])
        ...
        lead = self.env['crm.lead'].browse(ctx['active_id'])
...
class CrmLead(models.Model):
    ...
    via = fields.Selection([('direct', 'Direct'), ('agent', 'Agent')], 'Via', default=lambda *a: 'direct')
    agent_id = fields.Many2one('res.partner', 'Agent')
    lead_sequence = fields.Char('Lead Number', size=64, readonly=True)
    banquets_ids = fields.One2many('banquet.quotation.lead.history', 'ref_id', 'Banquet Quotation History')
    ...
    def unlink(self):
        ...
        for rec in self.browse():
            raise exceptions.except_orm(_('Invalid action !'), _('Cannot delete these Lead.!'))
...
class DepositPaymentPolicy(models.Model):
    ...
    name = fields.Char('Policy Name', size=64, required=True)
    ...
    company_id = fields.Many2one(related='shop_id.company_id', relation='res.company', string='Company', store=True)
...
class ThemePlan(models.Model):
    ...
    name = fields.Char('Theme Name', size=64, required=True)
...
class BanquetQuotation(models.Model):
    ...
    def unlink(self):
        ...
        """
        for rec in self.browse():
            raise exceptions.except_orm(_('Invalid action !'), _('Cannot delete these Quotation.!'))
    ...
    def onchange_pricelist_id(self):
        ...
        print("self.pricelist_id:::::::::::::::::::", self.pricelist_id)
        # warning = {
    ...
    def create(self, vals):
        # function overwrites create method and auto generate request no.
        req_no = self.env['ir.sequence'].next_by_code('banquet.quotation') or 'New'
        vals['name'] = req_no
    ...
    def _get_total_amt_pur(self):
        ...
        res = {}
        for obj in self:
            print("obj.pur_untax_amt:::::::::::::;", obj.pur_untax_amt)
    ...
    def _amount_line_tax_pur(self, line):
        ...
        taxes = line.pur_tax_ids.compute_all(
            line.cost_price_unit * (1 - (line.discount or 0.0) / 100.0), None, 1)
    ...
    def _amount_line_tax(self, line):
        ...
        taxes = line.taxes_id.compute_all(
            line.price * (1 - (line.discount or 0.0) / 100.0), None, line.number_of_days)
        val = taxes['total_included'] - taxes['total_excluded']
        return val
    ...
    def count_total_days(self):
        for obj in self:
            if obj.company_id.id == self.env.user.company_id.id:
                ...
                check_in = obj.checkin_date
                check_out = obj.checkout_date
                day_count1 = (check_out - check_in).days + 1
                if not policy_browse[0].name == '24hour':
                    wd_count = 0
                    time_con = str(policy_browse[0].time)
                    check_out_cons = obj.checkin_date + ' ' + time_con + ':00:00'
                    for count in range(0, (int(day_count1))):
                        single_date = check_in + relativedelta(days=count)
                        if str(single_date)[0:10] == obj.checkin_date[0:10]:
                            time_con = str(policy_browse[0].time)
                            check_out_cons = obj.checkin_date + ' ' + time_con + ':00:00'

                            if obj.checkin_date < check_out_cons:
                                wd_count += 1
                            if obj.checkout_date > check_out_cons:
                                wd_count += 1
                        elif str(single_date)[0:10] == obj.checkout_date[0:10]:
                            time_con = str(policy_browse[0].time)
                            check_out_cons = obj.checkout_date[0:10] + ' ' + time_con + ':00:00'
                            if obj.checkout_date > check_out_cons:
                                wd_count += 1
                        if (str(single_date)[0:10] != obj.checkin_date[0:10]) and (
                                str(single_date)[0:10] != obj.checkout_date[0:10]):
                            time_con = str(policy_browse[0].time)
                            check_out_cons = obj.checkin_date[0:10] + ' ' + time_con + ':00:00'
                            if obj.checkout_date > check_out_cons:
                                wd_count += 1
                    day_count1 = wd_count
                else:
                    day_count = (check_out - check_in).days
                    time_in = obj.checkin_date
                    time_out = obj.checkout_date
    ...
    def _get_default_shop(self):
        ...
        shop_ids = self.env['sale.shop'].search([('company_id', '=', company_id)])
        if not shop_ids:
            raise exceptions.except_orm(_('Error!'), _('There is no default shop for the current user\'s company!'))
        return shop_ids[0]
    ...
    name = fields.Char("Quotation No.", size=128, readonly=True)
    ...
    #                'tour_name':fields.char('Banquet Name',size=64,readonly=True, states={'draft': [('readonly', False)]},),
    mobile = fields.Char('Mobile Number', size=15, required=True, readonly=True,
    ...
    checkin_date = fields.Datetime("Prefer start Date", required=True, readonly=True,
                                   states={'draft': [('readonly', False)]})
    checkout_date = fields.Datetime("Prefer End Date", required=True, readonly=True,
                                    states={'draft': [('readonly', False)]}, )
    ...
    other_items_ids = fields.One2many('other.items', 'banquet_id', 'Other Items')
    ...
    number_of_rooms = fields.Integer(compute='count_total_rooms', string="Number Of Rooms", size=30)
    ...
    theme_id = fields.Many2one('theme.plan', 'Theme', required=True, readonly=True, states={
        'draft': [('readonly', False)]}, )
    board_toread = fields.Char("Board to Read", size=300, readonly=True, states={
        'draft': [('readonly', False)]}, )
    seating_id = fields.Many2one('seating.plan', 'Seating Plan', required=True, readonly=True, states={
        'draft': [('readonly', False)]}, )
    sale_untax_amt = fields.Float(compute='_get_untax_amt_sale', string="Sale Untaxed Amount")
    sale_tax_amt = fields.Float(compute='_get_tax_amt_sale', string="Sale Taxes ")
    pur_untax_amt = fields.Float(compute='_get_untax_amt_pur', string="Purchase Untaxed Amount")
    pur_tax_amt = fields.Float(compute='_get_tax_amt_pur', string="Purchase Taxes ")
    sale_total_amt = fields.Float(compute='_get_total_amt_sale', string="Sale Total Amount")
    pur_total_amt = fields.Float(compute='_get_total_amt_pur', string="Purchase Total Amount")
    ...
    percentage = fields.Float("Percentage/Deposit Amount", readonly=True, states={'draft': [('readonly', False)]}, )
    min_dep_amount = fields.Float("Minimum Deposit Amount", readonly=True, states={'draft': [('readonly', False)]}, )
    ...
    pricelist_id = fields.Many2one('product.pricelist', 'Pricelist', required=True, readonly=True,
                                   states={'draft': [('readonly', False)]})
    shop_id = fields.Many2one('sale.shop', 'Shop', required=True, readonly=True, states={'draft': [(
    ...
    @api.onchange('shop_id', 'checkin_date', 'checkout_date')
    def on_change_checkout(self):
        ...
        #             raise exceptions.except_orm('Warning', 'Shop is not selected.')

        if not self.checkout_date:
            self.checkout_date = time.strftime('%Y-%m-%d %H:%M:%S')
        if self.checkin_date and self.shop_id.id:
            ...
            a = self._cr.fetchone()
            if not a[0]:
                raise Warning('Deposit policy is not define.')
            #                 raise exceptions.except_orm('Configuration Error', 'Deposit policy is not define.')
            pay_obj = self.env['deposit.payment.policy'].search([('start_date', '=', a[0])])
            val = {'value': {
    ...
    def action_confirm(self):
        for obj in self:
            if not obj.room_ids:
                raise exceptions.except_orm(_("Warning"), _("Room Details are missing."))
            for room in obj.room_ids:
                room_line_id = self.env['hotel.room'].search([('product_id', '=', room.room_number.id)])
                ...
                if room_line_browse.room_folio_ids:
                    for history in room_line_browse.room_folio_ids:
                        if history.state == 'done':
                            ...
                            reservation_start_date = obj.checkin_date
                            reservation_end_date = obj.checkout_date
    ...
    def action_approve(self):
        for obj in self:
            ...
            rejected_history_id = self.env['banquet.quotation.lead.history'].search([('ref_id', '=', id_lead.id)])
    ...
    def action_create_tour(self):
        for obj in self:
            ...
            if not self.shop_id:
                raise exceptions.except_orm('Configuration Error', 'Shop is not define.')
            ...
            banquet_id = self.env['hotel.reservation'].create({
                ...
                'checkin': obj.checkin_date,
                'checkout': obj.checkout_date,
                'adults': obj.adult,
                'childs': obj.child,
            ...
            for room in obj.food_items_ids:
                ...
                for tax_line in room.tax_id:
                    tax_ids.append(tax_line.id)
                room_booking_id = self.env['food.items'].create({
            ...
            print("\n\n\n banquet_id =====", banquet_id.id)
            tour_search = self.env['hotel.reservation'].search([('id', '=', banquet_id.id)])
...
class HotelReservationLine(models.Model):
    ...
    def count_total_days(self):
        ...
        for obj in self:
            if obj.company_id.id == self.env.user.company_id.id:
                policy_obj = self.env['checkout.configuration'].search([('shop_id', '=', obj.line_id.shop_id.id)])
                # print("obj:::::::::",policy_obj,obj.line_id.shop_id.id)
                if not policy_obj:
                    raise UserError('Configuration Error! Checkout policy is not define for selected shop.')
                policy_browse = self.env['checkout.configuration'].browse(policy_obj.id)
    ...
    def count_price(self):
        ...
        for line in self:
            if line.line_id:
                ...
                print("\n\n\n price ======", line.number_of_days)
                taxes = line.taxes_id.compute_all(price, None, line.number_of_days)
                val = taxes['total_excluded']
                cur = line.line_id.pricelist_id.currency_id
                line.update({
                    'sub_total1': cur.round(val),
                })
                print("\n\n\n if val ======", val)
            else:
                print("inside else=======")
    ...
    def _amount_line_cost(self):
        ...
        for line in self:
            if line.banquet_id:
                ...
                taxes = line.pur_tax_ids.compute_all(price, None, line.number_of_days)
                val = taxes['total_excluded']
                user_id = res_user.browse(self._uid)
                cur = user_id.company_id.currency_id
            line.update({
    ...
    def onchange_room_id(self):
        print("ffffffffffffffff222222222222222222222222222")
        if self.line_id:
        ...
        print("\n\n\n room_id =====", room_id, room_id.id)
        v = {}
        res_list = []
        warning = ''
        if self.banquet_id:
            print("vcccccccccccccccccccccccc")
            if not pricelist:
                raise exceptions.except_orm(_("Warning"), _("PriceList is not Selected !"))
            ...
            if price is False:
                raise exceptions.except_orm(_("Warning"), _("Couldn't find a pricelist line matching this product!"))
            v['price'] = price
            tax_ids = []
            ...
            cost_price_unit = get_price(self, pricelist, product_browse.standard_price)
            print("\n\n\n cost_price_unit ======", cost_price_unit)
            v['cost_price_unit'] = cost_price_unit

            room_line_id = self.env['hotel.room'].search([('product_id', '=', room_id.id)])
            print("\n\n\n room_line_id =======", room_line_id)
            room_line_browse = self.env['hotel.room'].browse(room_line_id.id)
            print("\n\n\n room_line_browse =======", room_line_browse)
            if room_line_browse.room_folio_ids:
                for history in room_line_browse.room_folio_ids:
                    if history.state == 'done':
                        history_start_date = history.check_in
            ...
            if self.line_id:
                ...
                if self.room_number:
                    ...
                    print("self.number_of_days:::::::::", self.number_of_days, product_browse.uom_id.id, self.checkin)
                    v['price'] = price
                    tax_ids = []
                    for tax_line in product_browse.taxes_id:
                        tax_ids.append(tax_line.id)
    ...
    sub_total = fields.Float(compute='count_price', string="sub total", size=30, compute_sudo=True)
    cost_price_unit = fields.Float('Cost Price', digits='Cost Price')
    cost_price_subtotal = fields.Float(compute='_amount_line_cost', string='Cost Subtotal', digits='Cost Price')
    pur_tax_ids = fields.Many2many('account.tax', 'pur_hotel_line_tax_rel', 'hotel_line_id', 'tax_id', 'Purchase Taxes')
...
class HotelReservation(models.Model):
    ...
    def _amount_other_tax(self, line):
        val = 0.0
        taxes = line.tax_id.compute_all(line.price_subtotal * (1 - (line.discount or 0.0) / 100.0), None, 1)
        val = taxes['total_included'] - taxes['total_excluded']
        return val
    ...
    def _get_total_rental_cost(self):
        ...
        for obj in self:
            ...
            for line in obj.reservation_line:
                total += line.sub_total1
                # print("total===========================_get_total_rental_cost==========================", total)
                val += self._amount_line_tax(line)
                # print("val===========================_get_total_rental_cost==========================", val)
            for line in obj.food_items_ids:
                total += line.price_subtotal
                val += self._amount_food_tax(line)
            for line in obj.other_items_ids:
            ...
            #             })
            obj.total_cost1 = sum
        print("\n\n\n _get_total_rental_costttttttttttt")
        ...
        for obj in self:
            if obj.deposit_policy == 'percentage':
                for rental_line in obj.reservation_line:
                    room_line_id = self.env['hotel.room'].search([('product_id', '=', rental_line.room_number.id)])
                    if room_line_id:
                        room_line_browse = self.env['hotel.room'].browse(room_line_id)[0].id
                        print(room_line_browse, '===============room_line_browse=========')
                        if room_line_id.deposit_bool:
                            print(rental_line.sub_total1, "rental_line.total_rent")
                            total += (rental_line.sub_total1 + self._amount_line_tax(
                                rental_line)) * obj.percentage / 100
                print(total, "total$$$$$$$$$$$$$$$$$$4444")
    ...
    def on_change_checkout(self):
        ...
        if not self.checkout_date:
            self.checkout_date = time.strftime('%Y-%m-%d %H:%M:%S')
        addDays = datetime.datetime(*time.strptime(self.checkout_date, '%Y-%m-%d %H:%M:%S')[:5]) + delta
        if self.checkin_date:
            new_date = str(self.checkin_date)
            self._cr.execute('select max(start_date) from deposit_payment_policy where start_date <= %s ', (new_date,))
            a = self._cr.fetchone()
            if not a[0]:
                raise exceptions.except_orm('Configuration Error', 'Deposit policy is not define.')
            pay_obj = self.env['deposit.payment.policy'].search([('start_date', '=', a[0])])
    ...
    food_items_ids = fields.One2many('food.items', 'food_items_id', 'Food Items')
    other_items_ids = fields.One2many('other.items', 'other_items_id', 'Other Items')
    untaxed_amt = fields.Float(compute='_get_subtotal_amount', size=512, string="Untaxed Amount")
    ...
    total_cost1 = fields.Float(compute='_get_total_rental_cost', string="Total Reservation cost", )
    deposit_cost = fields.Float(compute='_get_deposit_cost1', string="Deposit Cost", )
    banquet_id = fields.Many2one('banquet.quotation', 'Booking Ref.', readonly=True)
    ...
    def done(self):
        ...
        for reservation in self:
            ...
            if reservation.deposit_cost2:
                ...
                data_obj = self.env['ir.model.data']
                data_id = data_obj._get_id('banquet_managment', 'deposit_journal_entry_wizard')
                view_id1 = False
                if self._context is None:
                    self._context = {}
                ctx = dict(self._context)
                ...
                print("\n\n in done create folio =======")
                so = self.create_folio()
                print("\n\n in done create folio === so====", so)
                return so
...
class FoodItems(models.Model):
    ...
    def _amount_line(self):
        ...
        for line in self:
            ...
            if line.food_items_id:
                taxes = line.tax_id.compute_all(price, None, line.product_uom_qty)
                val = taxes['total_excluded']
    ...
    def _amount_line_cost(self):
        ...
        for line in self:
            if line.banquet_id:
                price = line.cost_price_unit
                taxes = line.pur_tax_ids.compute_all(price, None, line.product_uom_qty)
                val = taxes['total_excluded']
                user_id = res_user.browse(self._uid)
                cur = user_id.company_id.currency_id
            #             line.update({
    ...
    def onchange_product_id(self):
        res = {}
        pricelist = self.food_items_id.pricelist_id.id
        if not pricelist:
            print("hfjdshfjdhfkjdhfdjfhfhsfd122222222212121212121212121212", pricelist, self.banquet_id)
            raise exceptions.except_orm('Warning', 'Price List is not define1.')
        if self.product_id:
            ...
            res['cost_price_unit'] = get_price(self, pricelist, record.standard_price)
    ...
    def get_subcategory_ids(self, parent_id):
        ...
        get_ids = []
        if parent_id:
            categ_sub_ids = self.env['product.category'].search([('parent_id', '=', parent_id)])
            if categ_sub_ids:
    ...
    def get_category_id(self):
        ...
        categ_ids = self.env['product.category'].search([('parent_id', '=', obj[0])])
        if categ_ids:
    ...
    price_unit = fields.Float('Unit Price', required=True, digits='Sale Price', default=0.0)
    price_subtotal = fields.Float(compute='_amount_line', string='Subtotal', digits='Sale Price')
    tax_id = fields.Many2many('account.tax', 'food_item_tax', 'order_line_id', 'tax_id', 'Taxes')
    product_uom_qty = fields.Float('Quantity (UoM)', digits='Product UoS', default=1)
    ...
    cost_price_unit = fields.Float('Cost Price', digits='Cost Price')
    cost_price_subtotal = fields.Float(compute='_amount_line_cost', string='Cost Subtotal', digits='Cost Price')
    pur_tax_ids = fields.Many2many('account.tax', 'pur_tax_tax_line_rel', 'food_id', 'tax_id', 'Purchase Taxes')
    purches_bol = fields.Boolean('Show Purchase Tax')
    category_id = fields.Char('Category', size=20, default='get_category_id')
...
class OtherItems(models.Model):
    ...
    def onchange_product_id(self):
        res = {}
        # banquet_id=self.env['banquet.quotation'].search([])
        # print("iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii",pk)
        pricelist = self.other_items_id.pricelist_id.id
        if not pricelist:
    ...
    def _amount_line_cost(self):
        ...
        for line in self:
            if line.banquet_id:
                ...
                cur = user_id.company_id.currency_id
            #             line.update({
            #                 'cost_price_subtotal': cur.rount(val),
            #             })
            line.cost_price_subtotal = cur.round(val)
    ...
    price_unit = fields.Float('Unit Price', required=True, digits='Sale Price', default=0.0)
    price_subtotal = fields.Float(compute='_amount_line', string='Subtotal', digits='Sale Price')
    tax_id = fields.Many2many('account.tax', 'other_service_tax', 'order_line_id', 'tax_id', 'Taxes')
    product_uom_qty = fields.Float('Quantity (UoM)', digits='Product UoS', default=1)
    product_uom = fields.Many2one('uom.uom', 'UoM', required=True)
    discount = fields.Float('Discount (%)', digits=(16, 2), deafult=0.0)
    banquet_id = fields.Many2one('banquet.quotation')
    cost_price_unit = fields.Float('Cost Price', digits='Cost Price')
    cost_price_subtotal = fields.Float(compute='_amount_line_cost', string='Cost Subtotal', digits='Cost Price')
    pur_tax_ids = fields.Many2many('account.tax', 'pur_other_tax_line_rel', 'other_id', 'tax_id', 'Purchase Taxes')
    purches_bol = fields.Boolean('Show Purchase Tax')
...
class BanquetQuotationLeadHistory(models.Model):
    ...
    name = fields.Char("Banquet Quotation No.", size=128, readonly=True)
