...
class transport_partner(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('confirm', 'Confirmed')], 'State', readonly=True, size=30, default='draft')
...
class hotel_reservation(models.Model):
    ...
    def confirmed_reservation(self):
        ...
        project_name = self.env['project.project'].search([('name', '=', 'Tranportation')])
        for reservation in self:
    ...
    def update_history(self):
        ...
        for reservation in self:
            if reservation.pick_up == 'yes':
                trans_id = self.env['transport.task'].search([('reservation_id', '=', reservation.id)])
                if trans_id:
...
class transport_task(models.Model):
    ...
    project_id = fields.Many2one('project.project', 'Project', ondelete='set null', default=get_project_id)
    ...
    def action_close(self):
        ...
        for obj in self:
            if obj.reservation_id:
                product_id = self.env['product.product'].search([('name', '=', 'Taxi')])
                if not product_id:
                ...
                if obj.reservation_id.state != 'done':
                    raise Warning(_("Warning! Folio is not created for this reservation."))
                folio = self.env['hotel.folio'].search([('reservation_id', '=', obj.reservation_id.id)])
                folio_id = folio
                acc_id = obj.guest_id.property_account_receivable_id.id
                if not acc_id:
                    raise Warning(_("Warning! Account Receivable is not defined for guest."))
                ...
                if not price_ids:
                    raise Warning(_("Warning! Pricing is not defined for these location and transport mode."))
                price_id = price_ids
                sale_price = get_price(self, obj.reservation_id.pricelist_id.id, price_id.sale_price)
                if obj.service_type == 'internal' and obj.reservation_id.chargeable and obj.reservation_id.pick_up:
                    ...
                    service_line_id = self.env['hotel_folio_transport.line'].create(service_line)
                if obj.service_type != 'internal' and obj.reservation_id.chargeable and obj.reservation_id.pick_up:
                    so_line = {
                        'product_id': product.id,
                        'name': product.name,
                    ...
                    service_line_id = self.env['hotel_folio_transport.line'].create(service_line)
                    cost_price = get_price(self, obj.trans_partner_id.partner_id.property_product_pricelist.id, price_id.cost_price)
                    if not obj.trans_partner_id.partner_id.property_account_payable_id:
                        raise Warning(_("Warning! Accounting property is not define for Transport Supplier."))
                    ...
                    if not product.property_account_expense_id:
                        ...
                        if not product.categ_id.property_account_expense_categ_id.id:
                            raise Warning(_("Warning! Accounting property is not define for Taxi."))
                    tax_ids = []
                    for tax_line in product.taxes_id:
                        tax_ids.append(tax_line.id)
                    invoice_line = {
                ...
                raise Warning(_("Warning! Reservation Ref is missing  for this task"))
