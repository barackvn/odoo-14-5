...
class Hotel_menucard(models.Model):
    ...
    product_id = fields.Many2one('product.template', string='Product_id', required=True, ondelete='cascade')
...
class hotel_restaurant_tables(models.Model):
    ...
    name = fields.Char(string='Table number', size=64, required=True)
...
#         return True
    ...
    def table_reserved(self):
        ...
        for reservation in self:
            ...
            self._cr.execute("select count(*) from hotel_restaurant_reservation as hrr "
                             ...
                             "inner join reservation_table as rt on rt.reservation_table_id = hrr.id "
                             "where hrr.id= %s) and hrr.state not in ('order','cancel')", (reservation.start_date, reservation.end_date, reservation.id, reservation.id))
            ...
            if roomcount:
                raise Warning('You tried to confirm reservation with table those already reserved in this reservation period')
            else:
    ...
    name = fields.Char('Reservation No', size=64, readonly=True)
...
class hotel_restaurant_kitchen_order_tickets(models.Model):
    ...
    orderno = fields.Char('KOT Number', size=64, readonly=True)
    resno = fields.Char('Order Number', size=64)
            ...
            if not obj.room_no:
                ...
                for ol in obj.order_list:
                    ...
                    if ol.name.categ_id:
                        ...
                        if not a:
                            raise ValidationError(_('There is no expense account defined for this product: "%s" (id:%d)') % (
...
class hotel_reservation_order(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', 'Customer', required=True)
    guest_name = fields.Char('Guest Name', size=164)
    room_no = fields.Many2one('hotel.room', 'Room No')
    folio_id = fields.Many2one('hotel.folio', 'Hotel Folio Ref')
    ...
    def create_invoice(self, *args):
        ...
        for obj in self:
            ...
            for ol in obj.order_list:
                ...
                print ("\n\n\n cres=====", cres)
            if obj.room_no:
                self._cr.execute('insert into order_reserve_invoice_rel(folio_id,invoice_id) values (%s,%s)', (obj.folio_id.id, inv_id.id))
        self.write({'state': 'done'})
        return True
